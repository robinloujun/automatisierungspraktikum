function f_approx(cmd,arg1)
% Original: NND11GN Generalization demonstration.
...
% Hinweis: Für die zugehörigen Beiblätter in WBS (WBS 7-18; 7-19) werden die Grafiken ab Linie 490 erzeugt
...
%disp('Beginn der Rechnervorführung')
% Löschen von Fehlermeldungen, die beim Programmablauf auftreten das Programm aber nicht beeinflussen
disp(' ')
home
...
elseif strcmp(cmd,'init') & (~fig)
  ...
  % NEW DEMO FIGURE
  fig = f_approx_f(me,'Funktionsapproximation','','Automatisierungstechnisches','Praktikum WS 2003-2004');
  set(fig, ...
  ...
  y = 115;
  len = 320;
  text(x,y,'Anzahl der Neuronen S1:',...
    'color',nndkblue,...
    'fontw','bold',...
    'fontsize',12,...
...
elseif strcmp(cmd,'train') & (fig) & (nargin == 1)
  ...
  T = 1 + sin(i*pi*P/4);
  [R,Q] = size(P);
  P2 = -2:(.04/i):2; %%Feine Auflösung für Darstellung von Overfitting und so
%  P2 = -2:(.4/i):2  %%Grobe Auflösung für Darstellung der Beiträge der Neuronen
...
semilogy(F2,'LineWidth',linesize)
groesse = 14;
title('Fehlerverlauf','FontSize',groesse+2)
ylabel('Fehler','FontSize',groesse)
xlabel('Trainingsschritte','FontSize',groesse)
